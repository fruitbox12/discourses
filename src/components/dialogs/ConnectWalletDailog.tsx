...
const ConnectWalletDailog = ({ open, setOpen }: { open: boolean, setOpen: Dispatch<SetStateAction<boolean>> }) => {
	...
	const [verifySig, { data: sigData, loading: sigLoading, error: sigError }] = useMutation(VERIFY_SIG, {
        onCompleted: (data) => {
            setMinting(false);
			dispatch(setUser({
    ...
    useEffect(() => {
		if (sigData) {
			setMinting(false);
			dispatch(setUser({
				token: sigData.verifySignature.token,
				username: sigData.verifySignature.username,
				walletAddress: sigData.verifySignature.address,
				expiresAt: new Date(new Date().getTime() + 1).toISOString(),
				isLoggedIn: true,
                tConnected: false,
                t_handle: "",
                        ...
                        {!minting && <>
                            ...
                            <Dialog.Description className="flex flex-col  w-full items-center  gap-4 text-center justify-between mt-4">
                                ...
                                <div className='flex items-center justify-center w-full gap-4'>
                                    <button ref={buttonRef} onClick={handleConnectWallet} className='button-s font-semibold tracking-wide px-6 py-3  text-xs bg-[#212427] rounded-lg outline-none'>Connect Wallet</button>
                                </div>
                            </Dialog.Description>
                        </>
                        }
